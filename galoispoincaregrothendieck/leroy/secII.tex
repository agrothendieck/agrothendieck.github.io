...
\begin{enumerate}
    ...
    $$
    soit commutatif, ou encore que soit commutative le diagramme
    $$
        \xymatrix{
            I_D \ar[r] & I_C \\
            D \ar[r] \ar[u]^{\beta\circ (1_D,g)} %% I think it is \beta here, the scan is unclear.
...
\vskip .3cm
{
Proposition {\bf 2.3.2.} --- \it Soient $A$ et $B$ deux objets connexes de $E$ tel que $A \times B$ soit non-vide. Si $A$ et $B$ se trivialisent l'un l'autre, ils sont isomorphes.
}
\vskip .3cm
...
\begin{enumerate}
    ...
    \item[(ii)] La restriction à $\LC(E, Y)$ du foncteur image directe $u_*$ [] $\phi$ est pleinement fidèle. %% this might be a \simeq
    D'où les corollaires:
    \item[(iii)] Les foncteurs $\phi_{/\LC(E, Y)}$ et $u^{-1}$ fournissent des équivalences quasi-inverses
    $$
    \LC(E,Y) \leftrightarrows B^G
    $$
...
\vskip .3cm
{
Proposition {\bf 2.4.2.} --- \it Soient $T$ un $\cU$-topos, $K$ une sous-catégorie pleine de $T$, $S$ la sous-catégorie pleine de $T$ formée des objets qui sont sommes directes d'objets de $K$. Supposons que:
\begin{enumerate}
...
Pour tout $i \in I$, soit $M_i \hookrightarrow Q$ l'image de $\Pi_{|_{C_i}}$. On a $C_i \times_{M_i} C_i = C_i \times_Q R_{i i}$, donc le diagramme
$$
    \xymatrix{
...
S'il existe en outre une petite famille $(Y_{\alpha})$ d'objets de $K$ telle que tout objet de $K$ puisse être recouvrement (dans $T$) par des $Y_{\alpha}$, alors $S$ est un $\cU$-topos et l'inclusion $S \to T$ définit un morphisme de topos $T \to S$ (par le critère de Giraud).
}

\vskip .3cm
{
Proposition {\bf 2.4.4.} --- \it Soient $L$ et $M$ deux objets de $E$. Tout objet connexe de $E$ qui trivialise $L$ et $M$ trivialise le faisceau $\underline{\Isom}(L, M)$ des germes d'isomorphisme de $L$ dans $M$. 
}
\vskip .3cm
...
\begin{enumerate}
    \item[(i)] Soient $S$ une catégorie $\cU$-petite et $I$ un ensemble $\cU$-petit. L'ensemble des préfaisceaux $F$ sur $S$ tels que $F(X) \subset I$ pour tout $X \in \Ob(S)$ est $\cU$-petit
    ...
    $$
    \Card(A) \leq \sum_{0 \leq n} c^{2n} \leq c^d
    $$
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item[b)] Supposons $u^{-1}(Y)$ galoisien. Soit $V$ l'image de $Y \to e_E$. $u^{-1}(V)$ est connexe et non-vide, et il existe un objet $U'$ de $A$ tel que 
        $$
        u^{-1}(V) \amalg U' \isom e_E
        $$

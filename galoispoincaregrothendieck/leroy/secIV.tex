...
$$
\xymatrix{
    e_{F_i} \ar[r] \ar[d]_a & e_{F_{[i]}} \ar[d]^a \\ %% should be a j ??
...
$$
le morphism induit par $\mu$ entre les restrictions à la fibre $F_i$. Comme ces restrictions sont représentables par $S(i)$ et $T(i)$ respectivement, $\mu_i$ provient d'un $i$-morphisme
$$
m_i: S(i) \to T(i)
$$
à savoir $m_i = \mu_{S(i)}(I_{S(i)})$

Prouvons que les $m_i$ définissent un morphisme $m: S\to T$ : Soit $i \xlongrightarrow{u} i$ une flèche de $i$. On a le diagramme commutatif
$$
    \xymatrix{
...
\vskip .3cm
{ 
Proposition (Sous-catégories génératrices de $F$) {\bf 4.4.3.} --- \it Soit $S$ une sous-catégorie pleine de $F$. Pour que $S$ engendre la site $F$ (i.e. que tout objet de $F$ puisse être recouvrement par des objets de $S$ au sens de la topologie $T$) il faut et il suffit que pour tout $i \in \Ob(I)$, la fibre $S_i$ engendre le topos $F_i$.
}
\vskip .3cm
...
$$
    \raisebox{2em}{\xymatrix{
        X_\alpha \times_X X_\beta \ar[r] \ar[d] & X_\beta \ar[d]^{\epsilon_\beta} \\
        X_\alpha \ar[r]_{\epsilon_\alpha} & Y
    }}
    \quad \text{(*)}
$$
...
$$
n(D_\lambda) \times_{n(C)} n(D_\mu) \isom n(D_\lambda \times_C D_\lambda) \quad \text{(cf 4.4.1, (ii))}
$$
...
$$
     \xymatrix{
         \Hom_D(D,H_D) \ar[r] \ar[d] & \Hom_D(C,H_D) \ar[d]\\
...
\vskip .3cm
{
Proposition (Image essentielle du foncteur $p$ (4.3)) {\bf 4.4.9.} --- \it Soit $G$ un faisceau sur $F$. Pour tout $i \in \Ob(I)$ représentons la restriction $G_{/F_i}$ par un objet $X_i$ de $F_i$, et soit $\epsilon_i \in G(X_i)$ la section qui défini l'isomorphisme $X_i \to G_{/F_i}$. Les propriétés suivantes sont équivalentes :
\begin{enumerate}
...
{\it Démonstration}. (i) $\Rightarrow$ (ii) Pour tout $i \in \Ob(I)$, désignons par $\alpha_i$ la section $i_{Y_i}$ de $p(Y)$ au-dessus de $Y(i)$ (4.3). Il existe un isomorphisme et un seul $Y(i) \to X_i$ qui rend commutatif le diagramme
 $$
     \xymatrix{
 ...
 (ii) $\Rightarrow$ (i) Soit $u: i \to j$ une flèche de $I$. Prouvons qu'il n'existe qu'une flèche $u$-cartésienne $m: X_i \to X_j$ qui rend commutatif le diagramme (*) : soit $m'$ une seconde flèche qui jouit de la même propriété. Il existe une $i$-automorphisme de $X_i$ et un seul, soit $\alpha$, tel que $m' = m \circ \alpha$. On a donc :
 $$
 \epsilon_i = \epsilon_j \circ n(m') = \epsilon_j \circ n(m) \circ n(\alpha) = \epsilon_i \circ n(\alpha)
 $$
 cela veut dire que $i_{X_i}$ et $\alpha$ ont même image par l'isomorphisme $X_i \isommap G_{/F_i}$\footnote{Plus précisément, par la bijection composée $\Hom_i(X_i, X_i) \isommap G(X_i) \isommap \Hom_{\widetilde{F}}(n(X_i), G)$}, d'où $m = m'$.
 
 Nous prouvons donc définir une section cartésienne $X: I \to F$ comme voici :
 \begin{enumerate}
     \item[a)] $X(i) = X_i$ pour tout $i \in \Ob(I)$
     \item[b)] pour toute flèche $i \xlongrightarrow{u} j$ de $I$, la flèche $u$-cartésienne $X(u): X_i \to X_j$ est déterminée par le diagramme commutatif (*) (en effet, l'unicité que l'on vient de prouver nous garantir que $X(v \circ u) = X(v) \circ X(u)$ pour tout couple de flèches $i \xlongrightarrow{u} j \xlongrightarrow{u} k$ de $I$). Et il est clair que $p(X)$ est isomorphe à G.
 \end{enumerate}
...
Il existe (4.5) de sections cartésiennes $S_\alpha$ de $F$ et des isomorphismes $p(S_\alpha) \isom F$. Nous définissons la catégorie $C$ comme voici : pour tout $i \in \Ob(I)$, a fibre $C_i$ est la sous-catégorie pleine de $C_i$ formée des composantes connexes des $S_\alpha(i)$.
...
Prouvons (i). Soient $C$ et $C'$ deux éléments de $J(F)$. On définit un minorant $D$ de $\{C, C'\}$ comme voici : étant donné $i \in \Ob(I)$, un objet $Z$ de $F_i$ est dans $D_i$ s'il existe un objet $Y$ de $C_i$ et un objet $Y'$ de $C'$ tels que $Z$ soit isomorphe à une composante connexe de $Y \times Y'$ (cf. 2.4.5, (ii)). 
